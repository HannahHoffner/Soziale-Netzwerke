...
#include <random> //für Seed
#include <fstream>
using namespace ns3;

NS_LOG_COMPONENT_DEFINE ("WIFINetworkAndLAN");
/*void TracePacketTx(std::string context, Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interface) {
    std::ofstream outFile;
    outFile.open("output.csv", std::ios::out | std::ios::app);
    outFile << Simulator::Now().GetSeconds() << "," << p->GetSize() << std::endl;
    outFile.close();
}*/
//void TracePacketTx(std::string context, Ptr<const Packet> p) {
void TracePacketTx(std::string context, Ptr<const Packet> p, Ptr<Ipv4> ipv4, uint32_t interface){

    static std::ofstream outFile("output.csv", std::ios::app); //out oder app(append, weiter schreiben)
    outFile << Simulator::Now().GetSeconds() << "," << context << "," << p->GetSize() << std::endl;
}
...
main(int argc, char* argv[])
{
    ...
    for (int seedIndex = 0; seedIndex < 10; ++seedIndex) {
...
/*
    ...
    // Setzen der TCP-Varianten für jeden Client
    TypeId tcpTypeId = (scenario == 1) ? TcpCubic::GetTypeId() : TcpBbr::GetTypeId();
    ...
    Simulator::Stop((scenario == 1) ? Seconds(300.0) : Seconds(20.0));
    ...
    std::string context = "/NodeList/" + std::to_string(wifiStaNodes.Get(i)->GetId()) + "/$ns3::Ipv4L3Protocol/Tx";
    Config::Connect(context, MakeCallback(&TracePacketTx));
}*/
    for (uint32_t i = 0; i < wifiStaNodes.GetN(); ++i) {
    std::string context = "Client-" + std::to_string(i);
    wifiStaNodes.Get(i)->GetObject<Ipv4>()->TraceConnect("Tx", context, MakeCallback(&TracePacketTx));
}

    std::ofstream outFile;
    outFile.open("output.csv", std::ios::app); //app --> alles in eine csv
    outFile << "Time,Client,BytesSent\n";
    outFile.close();
